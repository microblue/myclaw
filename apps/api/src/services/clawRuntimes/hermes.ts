...
server {
    ...
    location / {
        ...
        # Hermes dashboard rejects requests whose Host header doesn't
        # match the bound address (--host 127.0.0.1) — it returns
        # 400 "Invalid Host header" otherwise. Forward the loopback
        # name so the upstream is happy; the public TLS hostname is
        # already terminated by nginx.
