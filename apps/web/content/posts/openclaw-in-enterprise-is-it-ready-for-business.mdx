...
OpenClaw's authentication is built around Supabase Auth, which today is configured for email + password. Integrating with an enterprise IdP requires additional configuration and potentially custom work.

This is solvable -- Supabase supports SAML 2.0 and OIDC federation as well as third-party providers, and the OpenClaw architecture allows for authentication customization -- but it is not a plug-and-play experience with most enterprise SSO systems. Budget time for this integration if it is a requirement.
